...
Verify getting error page when city has multiple locations
...
sub test_two_cities: Test(no_plan)
{
	...
	ok($searchresultspage->is_page_title_present("Rent.com: Search Results"), 'Verify page title is Search');
	ok($searchresultspage->is_text_present("There are 2 'burbank's in CA. Please choose one."), 'Verify two city error message');
}
...
sub test_search_select_neighborhoods: Test(no_plan)
{
	...
	my $westwood = "Westwood Apartments";
	my $westwood_neighborhood = "146752836";
	...
	$searchresultspage->select_neighborhoods($westwood_neighborhood);
	
	ok($searchresultspage->is_page_title_present("Rent.com: Search Results"), 'Verify page title is Search');
	ok($searchresultspage->is_text_present($westwood), 'Verify this Westwood property is on the search results page');
